...
class PromptLearner(nn.Module):
    ...
    def forward(self, audio_features, session):
        ...
        return self.tokenized_prompts['input_ids'][:(session+1)*self.args.way,:].to(self.args.device), prompts_token_embeddings[:,:(session+1)*self.args.way,:,:], self.tokenized_prompts['attention_mask'][:(session+1)*self.args.way,:].to(self.args.device)
...
class CustomPENGI(nn.Module):
    ...
    def forward(self, audio, session=5):
        ...
        prompts_tokens, prompts_token_embeddings, prompts_attention_mask = self.prompt_learner(audio_features, session)
