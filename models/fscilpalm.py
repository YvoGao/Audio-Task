...
class PromptLearner(nn.Module):
    ...
    def forward(self, audio_features, text_features, session):

        lambdas = torch.sigmoid(self.lambdas[:(session+1)*self.args.way]).reshape(-1,1)  # [n_cls, 1]
        updated_text_features = (1-lambdas)*text_features + (lambdas*self.ctx[:(session+1)*self.args.way,:])     # [n_text_prompts, 1024]
...
class CustomPENGI(nn.Module):
    ...
    def forward(self, audio, session=5):

        audio_features = self.audio_encoder(audio)[0] # audio_features shape [n_audio_files, 1024]
        audio_features = audio_features / audio_features.norm(dim=-1, keepdim=True)
 

        prompts = [f"{class_name}" for class_name in self.args.classnames[:(session+1)*self.args.way]]
        ...
        text_features = self.prompt_learner(audio_features, text_features, session) # text_features shape [n_text_prompts, 1024]
