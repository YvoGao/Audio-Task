...
class CustomPENGI(nn.Module):
    ...
    def forward(self, audio, session=5):
        ...
        text = {"input_ids": prompts_tokens[:(session+1)*self.args.way,:], "inputs_embeds": prompts_token_embeddings[:(session+1)*self.args.way,:,:], "attention_mask": prompts_attention_mask[:(session+1)*self.args.way,:]}
